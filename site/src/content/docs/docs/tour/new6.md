...
To do this we're going to create an app to record the time spent on different activities throughout the day. You can code along in StackBlitz or follow the code here:
...
mount("main", ActivityTracker, [
  { name: "Moving", minutes: 0 },
  { name: "Idling", minutes: 0 },
]);
```
...
A framework creates a layer between you and the DOM, letting you focus on the high-level "what" of the application, while it deals with the "how". The "how" is full of subtle complexities, forcing the layer to becomes quite sophisticated, and as a result, opaque.

Working through a sophisticated and opaque layer comes with reduced **visibility** and **control**, which com back to bite you in several interesting ways as the project grows in complexity, and ultimately reduces your velocity.

Instead of one thick layer, Wallace uses several thin, dumb and very simple layers, which you can peel back to work at progressively lower levels. You can use this to take control, or briefly peek at underlying operations, thereby retaining full **visibility** and **control** and preserving your velocity.

To demonstrate this, we're going to unpack various bits of our app

> If you want to know how much Activity Tracker to record how much of your coding time lands in two categories:
>
> - **Moving** - you are typing away and being quite productive.
> - **Stalled** - you haven't typed for over a minute because you're stuck in "wait, what, why, how, but, what if, but then, hmmm..." territory.
>
> You'd notice something interesting. In the early stages of a project you spend most of your time moving, but as it grows more complex, the ratio shifts and your velocity eventually plummets.

## Unpacking

in the process you'll learn how Wallace works, how to use it, and how it helps you.

# ----------------------------------------------------

This magic is essentially a thick layer between you and the DOM which abstracts, automates and manages.

The problem with this thick layer is that you loose visibility and control.

We call it "magic" because it looks impressive and you don't fully understand how it works. This helps you develop faster by focusing on the _what_ and letting the framework figure out the _how_. But you've essentially handed control of the DOM over to an engine whose workings are hidden, and you pay a price for that later on.

## Time

There's a big difference between the time you think you spend on activities versus the time you actually spend on them. If you were to use our Activity Tracker to record how much of your coding time lands in two categories:

- **Moving** - you are typing away and being quite productive.
- **Stalled** - you haven't typed for over a minute because you're stuck in "wait, what, why, how, but, what if, but then, hmmm..." territory.

You'd notice something interesting. In the early stages of a project you spend most of your time moving, but as it grows more complex, the ratio shifts and your velocity eventually plummets. There are several reasons for this, and one of them (perhaps not the biggest, but it's certainly there) is the framework's engine.

The problem with an engine is that you loose **visibility** and **control**, which in turn:

1. Makes debugging harder
2. Causes (and hides) unintended behaviour
3. Forces you to do things a certain way, which can be restrictive.
4. Blocks you from doing certain things.
5. Runs at the speed it runs, and if that's too slow you may not have that many options.

You can sometimes bypass the engine and deal with the DOM directly, but that's messy at best.
...
Wallace solves this problem by layering several simple engines

A framework puts a layer between you and the DOM, which lets you focus the "what" without worrying about the "how" - and develop much faster as a result.

This layer is invariably ends up as thick, complex, clever layer whose internals are hidden. That's fine to begin with, but as your project grows you find this comes with downsides:
...
Wallace uses several thin layers which you can peel back to work at progressively lower levels of abstraction when needed. This could be:

- Customise behaviour or override defaults.
- Take control, possibly for performance reasons.
- Quickly check that the higher level abstraction is doing what you think it is.

This sounds rather abstract, so lets see this in practice.

Frameworks provide a high-level abstraction The problem is that they tend to bridge this gap in one big clever step.

Front-end frameworks create a layer between you and the DOM which lets you focus the high-level "what" without worrying about the low-level "how". This helps you develop apps faster, but also creates problems for later.

Because this layer deals with so much complexity on your behalf, it tends to be complex and opaque, which
